...
def calculate_similarity(compounds1, compounds2):
    ...
    # check each of the compounds lists for matching stoichiometries
    # convert the stoichiometries to sets first so order doesn't matter
    for i in range(len(compounds1)):
        compounds1[i]['Stoichiometry'] = set(compounds1[i]['Stoichiometry'])
    for j in range(len(compounds2)):
        compounds2[j]['Stoichiometry'] = set(compounds2[j]['Stoichiometry'])

    for i in range(len(compounds1)):
        for j in range(len(compounds2)):
            if compounds1[i]['Stoichiometry'] == compounds2[j]['Stoichiometry']:
                if compounds1[i]['Space Group'] == compounds2[j]['Space Group']:
                    similarity_index += 1

    return similarity_index/(len(compounds1) + len(compounds2)) # normalize similarity index and return
...
def main():
    ...
    json.dump(write_data, open('binary_similarities_with_sg.json', 'w'))
